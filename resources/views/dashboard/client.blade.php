        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100">
                <div class="p-6 lg:p-8">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Request a new appointment</h3>
                    @if($stylists->isEmpty())
                        <p class="text-gray-500">No stylists available yet. Please check back later.</p>
                    @else
                        <form action="{{ route('appointments.store') }}" method="POST" class="space-y-4 max-w-xl">
                            ...
                            <div>
                                ...
                                <select name="stylist_id" id="stylist_id" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-pink-500 focus:ring-pink-500">
                                    ...
                                    @foreach($stylists as $s)
                                        <option value="{{ $s->id }}" {{ old('stylist_id') == $s->id ? 'selected' : '' }}>{{ $s->name }}</option>
                                    @endforeach
                                </select>
                                @error('stylist_id')
                            ...
                            <div>
                                ...
                                <select name="service_id" id="service_id" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-pink-500 focus:ring-pink-500">
                                    ...
                                    @foreach($services as $sr)
                                        <option value="{{ $sr->id }}" {{ old('stylist_id') == $s->id ? 'selected' : '' }}>
                                            {{ $sr->name }} -
                                            {{ $sr->price }} KM -
                                            {{ $sr->avg_time }} Min 
                                        </option>
                            ...
                            <div>
                                <label for="appointment_at" class="block text-sm font-medium text-gray-700 mb-1">Date & time</label>
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100">
                <div class="p-6 lg:p-8">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Your appointments</h3>
                        ...
                        <ul class="divide-y divide-gray-200">
                            @foreach($appointments as $apt)
                                <li class="py-4 first:pt-0 last:pb-0 flex flex-wrap items-center justify-between gap-2">
                                    <div>
                                        <p class="font-medium text-gray-900">With {{ $apt->stylist->name }}</p>
